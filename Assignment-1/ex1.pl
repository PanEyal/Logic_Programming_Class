...
% continue checking for larger odd numbers that are smaller than
% sqrt(N).
is_prime(N,ODD_INT) :-
    sqrt(N) > ODD_INT,
    0 =\= N mod ODD_INT,
...
% check from value 3, and make sure N is right-prime.
right_prime_gen(N) :-
    right_prime_gen(N,3),
    right_prime(N).

% if N right prime and equal to the comapred ODD_INT it is a new right
% prime.
right_prime_gen(N,N).

% check for the next ODD_INT that may satisfy.
...
% split preorder and inorder lists correspondly and call in a recursive
% way for the two sub-trees
...
% if the operation is '+', postpond the evaluation because '*' has
% higher priority.
